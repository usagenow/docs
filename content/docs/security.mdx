...
- **Kiro and Warp** — UsageNow reads no credential for either. Kiro's limits come from its own log and Warp's activity from its local database, opened read-only.
- **API keys you add** — for DeepSeek, Kimi, and OpenRouter. A key is stored only in the Keychain on this Mac, sent over HTTPS only to that provider's host, used only for read requests, and deleted when you turn the provider off. Redirects aren't followed, and an answer from any other host is refused, so the key can't be passed on. See [API Key Providers](/providers/api-keys).
