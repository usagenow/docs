...
From 0.4.0, release builds send events — only while **Share anonymous usage analytics** is on — to `telemetry.usagenow.com`, a receiver run by UsageNow. It accepts exactly the fields listed above and refuses any event carrying anything else, so provider data couldn't be stored even if a future version sent it by mistake.

The receiver records the two-letter country Cloudflare resolves for the connection. It never records the IP address, request headers, or user agent, and events are deleted after 180 days.

Earlier releases carry no endpoint and send nothing, whatever the setting says.
